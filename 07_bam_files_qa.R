...
#cvSeqnames = c(paste0('chr', 1:19), 'chrX', 'chrY') # for mouse mm10
## choose the sequence names over which to calculate coverage
cvSeqnames = c(paste0('chr', 1:22), 'chrX', 'chrY') # for human hg38
...
q = paste0('select Sample.id as sid, Sample.group1, Sample.group2, Sample.group3, Sample.title, Sample.description, File.* from Sample, File
           where (Sample.idData = 49) AND (File.idSample = Sample.id AND File.type like "%_sort%")')
...
table(csFiles %in% dfSample$name)
...
setwd(gcswd)
n = make.names(paste('CBamScaffold pre duplicate removal data id 49 Manu Cardiac rds'))
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from bam files before duplicate removal for Manu Cardiac cohort PID 27', date())
...
setwd(gcswd)
n = make.names(paste('CBamScaffold after duplicate removal data id 49 manu cardiac rds'))
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from bam files after duplicate removal for manu cardiac PID 27', date())
n2 = paste0('~/Data/MetaData/', n)
#save(lAllBams, file=n2)
