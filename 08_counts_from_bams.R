...
q = paste0('select Sample.id as sid, Sample.group1, Sample.group2, Sample.title, File.* from Sample, File
           where (Sample.idData = 49) AND (File.idSample = Sample.id AND File.type like "%duplicates removed%")')
...
setwd(gcswd)
n = make.names(paste('lCounts rd did 49 manu reverse from bams rds'))
