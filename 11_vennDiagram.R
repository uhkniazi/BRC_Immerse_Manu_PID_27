...
lFiles = list.files('results/merged/', pattern='DEAnalysis*', full.names = T, ignore.case = T)

ldfData = lapply(lFiles, function(x) as.data.frame(read.csv(x, header=T, row.names=1, stringsAsFactors = F)))
names(ldfData) = lFiles
sapply(ldfData, nrow)

## remove the duplicated gene ids from the salmon sheet
## take average of FC for each gene based on transcripts
df = ldfData$`results/merged//DEAnalysis_SalmonT1VST0.xls`
df = df[df$ind %in% ldfData$`results/merged//DEAnalysis_HisatT1VST0.xls`$ind, ]
i = duplicated(df$ind)
table(i)
df = df[!i,]
rownames(df) = paste0('X', df$ind)
ldfData$`results/merged//DEAnalysis_SalmonT1VST0.xls` = df


# put everything in one order by row names
rn = rownames(ldfData[[2]])
head(rn)
rm(df)
...
sapply(ldfData, function(df) table(rownames(df) %in% rn))

sapply(ldfData, function(df) identical(rownames(df), rn))

cvTitle = gsub('results/merged//DEAnalysis_', '', names(ldfData))
...
venn.diagram(lVenn, filename = 'results/merged/venn_all_contrasts_T0VST1.tif', margin=0.1)
...
write.csv(dfCommonGenes, file='results/merged/commonDEGenes.xls')
