...
g_did
dfSample = dbGetQuery(db, "select * from Sample where idData=49;")
...
q = paste0('select File.*, Sample.id as sid, Sample.* from File, Sample where Sample.idData=49 AND Sample.id=File.idSample AND File.type="fastq"', ';')
...
cNames = paste(dfFiles$title, as.character(fReadDirection),  sep='_')
