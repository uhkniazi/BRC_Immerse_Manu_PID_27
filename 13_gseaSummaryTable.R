...
lFiles = list.files('results/hisat2/', pattern='*pathways_mSigDb_c2_*', full.names = T, ignore.case = F)
...
sn = gsub('results/hisat2//(.+VS.+)_up.+', '\\1', names(ldfData.up))
names(ldfData.up) = sn

sn = gsub('results/hisat2//(.+VS.+)_down.+', '\\1', names(ldfData.down))
...
lFiles = list.files('results/hisat2/', pattern='*pathways_mSigDb_c5.xls', full.names = T, ignore.case = F)
...
write.csv(dfMerged.c2, file='results/hisat2/gsea_msigdb_c2_merged.xls')
write.csv(dfMerged.c5, file='results/hisat2/gsea_msigdb_c5_merged.xls')
...
write.csv(dfMerged, file='results/hisat2/gsea_msigdb_significant_c2_c5_merged.xls')
...
pdf('results/hisat2/gsea_msigdb_significant_merged.pdf')
aheatmap(mMat, annRow = NA, scale = 'none', Rowv = order(g2:g1), Colv=NA, cexRow=5, cexCol = 0.6, #labCol=c('C2vC1', 'K1vC1', 'K2vC2', 'K2vK1'), 
         col=c('white', brewer.pal(9, 'YlOrRd')))
