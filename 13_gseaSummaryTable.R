...
lFiles = list.files('dataID50/results/', pattern='*pathways_mSigDb_c2_*', full.names = T, ignore.case = F)
...
sn = gsub('dataID50/results//(.+vs.+)_up.+', '\\1', names(ldfData.up))
names(ldfData.up) = sn

sn = gsub('dataID50/results//(.+vs.+)_down.+', '\\1', names(ldfData.down))
...
lFiles = list.files('dataID50/results/', pattern='*pathways_mSigDb_c5.xls', full.names = T, ignore.case = F)
...
write.csv(dfMerged.c2, file='dataID50/results/gsea_msigdb_c2_merged.xls')
write.csv(dfMerged.c5, file='dataID50/results/gsea_msigdb_c5_merged.xls')
...
write.csv(dfMerged, file='dataID50/results/gsea_msigdb_significant_c2_c5_merged.xls')
...
pdf('dataID50/results/gsea_msigdb_significant_merged_reactome.pdf')
i = grep('REACTOME', rownames(mMat))
m = mMat[i,]
...
pdf('dataID50/results/gsea_msigdb_significant_merged_GO.pdf')
i = grep('GO_', rownames(mMat))
m = mMat[i,]
#rownames(m) = gsub('REACTOME_', '', rownames(m))
aheatmap(m, annRow = NA, scale = 'none', Rowv = NA, Colv=NA, cexRow=1, cexCol = 1, #labCol=c('C2vC1', 'K1vC1', 'K2vC2', 'K2vK1'), 
         col=c('white', brewer.pal(9, 'YlOrRd')))
